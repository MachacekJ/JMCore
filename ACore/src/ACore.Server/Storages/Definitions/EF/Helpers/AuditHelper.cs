...
public class AuditHelper<TEntity, TPK>(IMediator mediator, IModel model, EFStorageDefinition storageDefinition, TEntity initData)
  where TEntity : PKEntity<TPK>
{
  ...
  public async Task InsertDbAction(TEntity savedData)
  {
    ...
    foreach (var d in diff)
    {
      ...
      if (colName != null)
        _preparedAuditEntryItem.AddColumnEntry(colName, d.Type.FullName ?? d.Name, d.IsChange, d.RightValue, d.LeftValue);
    }

    _preparedAuditEntryItem.SetPK(savedData.Id);
  ...
  public async Task UpdateDbAction(TEntity oldData)
  {
    ...
    foreach (var d in diff)
    {
      var colName = GetColumnName<TEntity>(d.Name, _dbEntityType);
      if (colName != null)
        _preparedAuditEntryItem.AddColumnEntry(colName, d.Type.FullName ?? d.Name, d.IsChange, d.LeftValue, d.RightValue);
    }

    // Compare(oldData, newData, p =>
    // {
