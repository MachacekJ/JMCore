...
public class AuditPKT : AuditAttributeBaseT
{
  ...
  public async Task GuidPK()
  {
    ...
    await RunTestAsync(method, async () =>
    {
      ...
      // Action 1
      await TestBasicPGDbContext.TestPKGuid.AddAsync(item);
      await TestBasicPGDbContext.SaveChangesAsync();

      // Assert 1
      Assert.True(await TestBasicPGDbContext.TestPKGuid.CountAsync() == 1);
  ...
  public async Task StringPK()
  {
    ...
    await RunTestAsync(method, async () =>
    {
      ...
      // Action 1
      await TestBasicPGDbContext.TestPKString.AddAsync(item);
      await TestBasicPGDbContext.SaveChangesAsync();

      // Assert 1
      Assert.True(await TestBasicPGDbContext.TestPKString.CountAsync() == 1);
