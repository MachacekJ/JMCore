...
public class LocalizeTableT : LocalizeBaseT
{
    ...
    public async Task CheckDatabase()
    {
        ...
        await RunTestAsync(method, async () =>
        {
            var allServer = await LocalizeDbContext.Localizations.Where(l => l.Scope.HasFlag(LocalizationScopeEnum.Server)).ToListAsync();
            ...
            var allClient = await LocalizeDbContext.Localizations.Where(l => l.Scope.HasFlag(LocalizationScopeEnum.Client)).ToListAsync();
